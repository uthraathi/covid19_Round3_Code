...
<!--                    <td>Volunteer ID</td>
                ...
                $total_pages_sql = "SELECT count(*) FROM volunteer_work_assign a, volunteer_registration V where v.user_id = '$USER_ID' AND V.Vol_ID = A.VOL_ID ORDER BY A.CATEGORY,A.SUB_CATEGORY,A.START_DT";
                $result_sql = mysqli_query($MyConnection,$total_pages_sql);
                $total_rows = mysqli_fetch_array($result_sql)[0];
                $total_pages = ceil($total_rows / $no_of_records_per_page);

                $sql = "SELECT A.CATEGORY,A.SUB_CATEGORY,A.WORK_TITLE,A.WORK_PDF,A.START_DT,A.END_DT,A.vol_id,V.name,V.MOBILE,V.EMAIL,WORK_ID FROM volunteer_work_assign a, volunteer_registration V where v.user_id = '$USER_ID' AND V.Vol_ID = A.VOL_ID ORDER BY A.CATEGORY,A.SUB_CATEGORY,A.START_DT LIMIT $offset,$no_of_records_per_page";
